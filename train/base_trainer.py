...
from train.saver import CheckpointSaver
from train.data_loader import CheckpointDataLoader
...
class BaseTrainer(object):
    ...
    def __init__(self, options):
        ...
        self.init_fn() # self.models_dict, self.optimizers_dict
        ...
        if checkpoint_file is not None:
            ...
            for model in self.models_dict:
                if model in checkpoint:
                    ...
                    print('Checkpoint %s loaded' % checkpoint_file)
    ...
    def train(self):
        ...
        for epoch in tqdm(range(self.epoch_count, self.options.num_epochs), total=self.options.num_epochs, initial=self.epoch_count):
            ...
            for step, batch in enumerate(tqdm(train_data_loader, desc='Epoch '+str(epoch),
                ...
                if time.time() < self.endtime:
                    ...
                    if self.step_count % self.options.summary_steps == 0:
                        self.train_summaries(batch, self.options, *out)
            ...
            if (epoch+1) % self.options.num_epochs == 0:
    ...
    def train_summaries(self, input_batch, options):
