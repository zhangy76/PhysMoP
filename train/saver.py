...
class CheckpointSaver():
    ...
    def save_checkpoint(self, models, optimizers, epoch, batch_idx, batch_size, dataset_perm, total_step_count):
        ...
        checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, timestamp.strftime('%Y_%m_%d-%H_%M_%S_') + '%d.pt'%total_step_count))
        checkpoint = {}
        for model in models:
    ...
    def load_checkpoint(self, models, optimizers, checkpoint_file=None):
        ...
        for model in models:
            if model in checkpoint:
                models[model].load_state_dict(checkpoint[model], strict=False)
        for optimizer in optimizers:
